...
const CURATED: Category[] = [
  { label: "DeFi & Trading", subcategories: [
    ...
    { label: "Perpetuals & derivatives", description: "Perpetual futures on Flash Trade", query: "perpetual futures leverage trading derivatives",
      recommendation: { skills: [], mcps: [{ name: "Flash Trade MCP", setup: "npx flash-trade-mcp" }, { name: "Perp CLI MCP", setup: "npx -y -p perp-cli perp-mcp" }], repos: [], tip: "Flash Trade for SOL/BTC/ETH perps. Perp CLI covers Pacifica + Hyperliquid." } },
    { label: "Stablecoins & payments", description: "Commerce Kit, stablecoin yields, checkout flows", query: "stablecoin payments USDC yield commerce",
...
const TAG_TO_SKILL: Record<string, { name: string; install: string }> = {
  ...
  perpetual: { name: "Flash Trade MCP", install: "npx flash-trade-mcp" },
